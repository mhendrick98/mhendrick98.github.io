(function($) {
  ...
  function getSelection(plugin, $a, $b) {
    ...
    if (
      !$a.hasClass("time-slot") ||
      !$b.hasClass("time-slot") ||
  ...
  DayScheduleSelector.prototype.attachEvents = function() {
    ...
    this.$el.on("click", ".time-slot", function() {
      ...
      if (!plugin.isSelecting()) {
        // if we are not in selecting mode
      ...
      plugin.$el.find(".time-slot[data-day='" + v + "']").each(function() {
        ...
        if (
          ...
          $(this).is(
            $(this)
              .closest("tbody")
              .find(".time-slot[data-day='" + v + "']:last")
          )
        ) {
          end = secondsSinceMidnightToHhmm(
            hhmmToSecondsSinceMidnight($(this).data("time")) +
              plugin.options.interval * 60
          );
        }
  ...
  function Plugin(option) {
    return this.each(function() {
      ...
      if (!data) {
        $this.data(
          "artsy.dayScheduleSelector",
          (data = new DayScheduleSelector(this, options))
        );
      }
    });
  }
  ...
  function timeDiff(start, end) {
    ...
    // need a dummy date to utilize the Date object
    return (
      (new Date(2000, 0, 1, end.split(":")[0], end.split(":")[1]).getTime() -
        new Date(
          2000,
          0,
          1,
